...
class SmartResumeAnalyzer:
    ...
    def calculate_intelligent_score(self, skills, verb_analysis, achievements, ats_score, sections_found):
        ...
        skills_score = min(3.0, sum(len(skill_list) for skill_list in skills.values()) * 0.15)

        # Action verbs score
        verb_score = min(2.0, (verb_analysis['strong_count'] * 0.3) - (verb_analysis['weak_count'] * 0.1))
        verb_score = max(0, verb_score)

        # Achievements score
        achievement_score = min(2.0, len(achievements) * 0.4)

        # Sections completeness score
        section_score = min(1.5, sum(sections_found.values()) * 0.2)

        # ATS compatibility score
        ats_contribution = min(1.5, ats_score / 100 * 1.5)

        total_score = skills_score + verb_score + achievement_score + section_score + ats_contribution
